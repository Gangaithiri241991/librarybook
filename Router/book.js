...
router.get("/all", async (req, res) => {
  try {
    ...
    const students = await getAllBook(req);
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const students = await getBookById(id);
    if (!students) {
      res.status(400).json({ data: "User Not found" });
      return;
    }
...
router.post("/add", async (req, res) => {
  try {
    ...
    const result = await addBookData(newStudent);
    res
      .status(200)
...
router.put("/edit/:id", async (req, res) => {
  try {
    ...
    const result = await updateBookData(id, updatedData);
    res
      .status(200)
...
router.delete("/delete/:id", async (req, res) => {
  try {
    ...
    const result = await deletaBookData(id);
    res
      .status(200)
